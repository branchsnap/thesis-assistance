# 简体中文版本摘要（冻结于 v3.21.2）

本文件是 `README.zh-CN.md` 在 2026-09-15 之前所载的版本摘要，原封搬出、不再更新。v3.21.2 之后的版本请看英文版 [CHANGELOG.md](../../CHANGELOG.md)；`README.zh-CN.md` 只保留最近三个版本。

_This file is the frozen copy of the Simplified Chinese release summaries that `README.zh-CN.md` carried until 2026-09-15. It is not updated; later releases are in the English [CHANGELOG.md](../../CHANGELOG.md)._
...
- **`academic-paper` SKILL + agent file 添加 `## v3.6.6 Generator-Evaluator Contract Protocol` 区块**（WORKFLOW.md 101 行 + `draft_writer_agent.md` 47 行 + `peer_reviewer_agent.md` 57 行）。WORKFLOW.md 另加 `## Known limitations` 区块承载 graceful-degradation + cross-session resume v3.6.7+ forward note。
...
- 所有顶层 `WORKFLOW.md` 添加 `metadata.data_access_level`，并以 `raw`、`redacted`、`verified_only` 为强制词汇。
- 所有顶层 `WORKFLOW.md` 添加 `metadata.task_type`，并以 `open-ended`、`outcome-gradable` 为强制词汇。
...
- 同步 README、`.claude/CLAUDE.md`、`MODE_REGISTRY.md` 与各 `WORKFLOW.md` 的 mode 数量与公开版本标注。
...
**Wave 3：精简 Skill 尺寸**
- WORKFLOW.md 总大小从 142KB 降至 85KB（-40%），详细协议移至 `references/` 按需加载
- 添加 ~15 个 reference 文件（re-review protocol、guided mode、systematic review、process summary 等）
- 所有 IRON RULE 保留在 WORKFLOW.md；详细内容按需加载
...
- 为 4 个 WORKFLOW.md 加入 `status: active` 和 `related_skills` 交叉引用
- 支持 skill 探索工具及跨技能导航
...
### v2.0.1 (2026-03)
- **精简 4 个 WORKFLOW.md**（-371 行, -16.5%）：移除跨 skill 重复、内嵌模板改为文件引用、冗余路由表、重复模式选择区块
