# 한국어 릴리스 요약 (v3.21.2에서 동결)

이 파일은 2026-09-15까지 `README.ko-KR.md`에 실려 있던 릴리스 요약을 그대로 옮긴 것입니다. 이후 갱신되지 않습니다. v3.21.2 이후의 릴리스는 영어판 [CHANGELOG.md](../../CHANGELOG.md)를 참조하세요. `README.ko-KR.md`에는 최근 3개 릴리스만 남습니다.

_This file is the frozen copy of the Korean release summaries that `README.ko-KR.md` carried until 2026-09-15. It is not updated; later releases are in the English [CHANGELOG.md](../../CHANGELOG.md)._
...
- **`academic-paper` SKILL + 에이전트 파일**이 축어적 `## v3.6.6 Generator-Evaluator Contract Protocol` 블록을 획득(WORKFLOW.md 101행 + `draft_writer_agent.md` 47행 + `peer_reviewer_agent.md` 57행). WORKFLOW.md는 v3.6.7+용 graceful-degradation + cross-session resume forward notes를 담는 새 `## Known limitations` 절도 추가.
...
- 모든 최상위 `WORKFLOW.md` 파일에 강제 어휘를 갖는 `metadata.data_access_level` 추가: `raw`, `redacted`, `verified_only`.
- 모든 최상위 `WORKFLOW.md` 파일에 강제 어휘를 갖는 `metadata.task_type` 추가: `open-ended`, `outcome-gradable`.
...
- README, `.claude/CLAUDE.md`, `MODE_REGISTRY.md`, `WORKFLOW.md` 파일을 현재 모드 수와 게시된 스킬 버전에 동기화.
...
- 상세 프로토콜을 `references/` 파일로 추출해 WORKFLOW.md 총 크기를 142KB에서 85KB로 축소(−40%)
- 약 15개의 새 참조 파일 생성(re-review 프로토콜, guided 모드, systematic review, process summary, external review 등)
- 모든 IRON RULE 마커는 WORKFLOW.md에 보존; 상세 콘텐츠는 온디맨드 로드
...
- 4개 WORKFLOW.md frontmatter 전부에 `status: active`와 `related_skills` 교차 참조 추가.
...
### v2.0.1 (2026-03)
- **4개 WORKFLOW.md 간소화**(-371행, -16.5%): 교차 스킬 중복 제거, 인라인 템플릿 → 파일 참조, 중복 라우팅 표, 중복 모드 선택 절
