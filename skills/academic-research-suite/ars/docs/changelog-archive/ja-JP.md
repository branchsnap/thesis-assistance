# 日本語版リリース要約（v3.21.2 で凍結）

このファイルは、2026-09-15 まで `README.ja-JP.md` に掲載されていたリリース要約をそのまま移したものです。以後は更新されません。v3.21.2 より後のリリースは英語版 [CHANGELOG.md](../../CHANGELOG.md) を参照してください。`README.ja-JP.md` には直近 3 リリースのみが残ります。

_This file is the frozen copy of the Japanese release summaries that `README.ja-JP.md` carried until 2026-09-15. It is not updated; later releases are in the English [CHANGELOG.md](../../CHANGELOG.md)._
...
- **`academic-paper` SKILL + エージェントファイル** が逐語的 `## v3.6.6 Generator-Evaluator Contract Protocol` ブロックを取得（WORKFLOW.md で 101 行 + `draft_writer_agent.md` で 47 行 + `peer_reviewer_agent.md` で 57 行）。WORKFLOW.md は v3.6.7+ 用の graceful-degradation + cross-session resume forward notes を運ぶ新しい `## Known limitations` セクションも追加。
...
- すべてのトップレベル `WORKFLOW.md` ファイルに強制語彙付き `metadata.data_access_level` を追加: `raw`、`redacted`、`verified_only`。
- すべてのトップレベル `WORKFLOW.md` ファイルに強制語彙付き `metadata.task_type` を追加: `open-ended`、`outcome-gradable`。
...
- README、`.claude/CLAUDE.md`、`MODE_REGISTRY.md`、`WORKFLOW.md` ファイルを現在のモードカウントと公開されたスキルバージョンに同期。
...
**Wave 3: リーンスキルサイズ**
- WORKFLOW.md 合計サイズを 142KB から 85KB（-40%）に削減、詳細プロトコルを `references/` ファイルに抽出
- ~15 の新しいリファレンスファイル作成（re-review プロトコル、ガイドモード、systematic review、process summary、external review など）
- すべての IRON RULE マーカーは WORKFLOW.md に保持。詳細コンテンツはオンデマンドでロード
...
- 4 つの WORKFLOW.md frontmatters すべてに `status: active` と `related_skills` クロスリファレンスを追加。
...
### v2.0.1 (2026-03)
- **4 つの WORKFLOW.md を簡素化**（-371 行、-16.5%）: クロススキル重複の削除、インラインテンプレート → ファイル参照、冗長ルーティングテーブル、重複モード選択セクション
